...
# --- 4. BANNER ---
st.markdown('<div style="background: linear-gradient(90deg, #0f0c29, #302b63, #24243e); padding: 20px; border-radius: 15px; text-align: center; border: 1px solid #00ffcc;"><h1 style="color: #00ffcc; margin: 0;">📊 MOMENTUM PRO V11</h1><p style="color: white; opacity: 0.8;">Sentinel AI Engine • Full Strategy</p></div>', unsafe_allow_html=True)
...
if df_rt is not None and not df_rt.empty:
    ...
    fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['BBU_20_2.0'], line=dict(color='rgba(173, 216, 230, 0.4)'), name='Upper BB'))
    fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['BBM_20_2.0'], line=dict(color='gray', dash='dash'), name='Mid BB'))
    fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['BBL_20_2.0'], line=dict(color='rgba(173, 216, 230, 0.4)'), fill='tonexty', name='Lower BB'))
    ...
    # Strength Meter (Con fix per riga 135)
    st.markdown("---")
    ...
    if df_d is not None and not df_d.empty:
        ...
        last_rsi, last_atr, last_adx = float(df_d['RSI'].iloc[-1]), float(df_d['ATR'].iloc[-1]), float(df_d['ADX'].iloc[-1])
        ...
        if not s_data.empty:
            if s_data.index[0] in pair[:3]: score += 20
